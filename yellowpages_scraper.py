#!/usr/bin/env python3
"""
Yellow Pages Scraper for Pool Cleaning and Maintenance Services
Scrapes business information across multiple states and saves to CSV files.
Uses playwright-stealth with fresh browser contexts per state.
"""
...
# STATES = ["CA", "TX", "AZ", "NY", "NJ", "PA", "OH", "MI", "MA"]
STATES = ["CA"]
...
def human_like_delay(min_sec=3.0, max_sec=15.0):
...
def scrape_page(page, state: str, page_num: int, retry_attempt: int = 0) -> tuple[list[dict], bool, bool]:
    ...
    try:
        ...
        human_like_delay(3.0, 12.0)

        # Simulate some scrolling behavior
        page.evaluate("window.scrollBy(0, 300)")
        human_like_delay(2, 4.3)
        page.evaluate("window.scrollBy(0, 200)")
        human_like_delay(3, 5.5)
...
def scrape_state_with_fresh_context(playwright, state: str) -> list[dict]:
    """Scrape all pages for a state, rotating browser context every 2-4 pages.

    Args:
        playwright: Playwright instance
        state: State code to scrape
    """
    all_businesses = []
    page_num = 1
    consecutive_empty = 0
        ...
        while True:
            ...
            if not businesses:
                ...
                if consecutive_empty >= max_consecutive_empty:
                    logger.info(f"Stopping {state} after {max_consecutive_empty} consecutive empty pages")
                    break
            else:
                consecutive_empty = 0
                all_businesses.extend(businesses)

            if not has_next and not businesses:
                break
            ...
            # Safety limit
            if page_num > 100:
                logger.warning(f"Reached page limit (100) for {state}")
                break
...
def save_to_csv(businesses: list[dict], state: str):
    """Save businesses to a CSV file for the given state."""
    if not businesses:
        logger.warning(f"No businesses to save for {state}")
        return

    filename = f"yellowpages_pool_services_{state}.csv"
...
def main():
    ...
    with sync_playwright() as p:
        for state in STATES:
            ...
            try:
                businesses = scrape_state_with_fresh_context(p, state)

                if businesses:
                    save_to_csv(businesses, state)
                    total_businesses += len(businesses)
                    logger.info(f"Completed {state}: {len(businesses)} businesses found")
                else:
            ...
            # Delay between states (new browser context)
            if state != STATES[-1]:
    ...
    logger.info(f"{'='*50}")
    logger.info(f"Total businesses scraped: {total_businesses}")
    logger.info(f"States processed: {len(STATES) - len(failed_states)}/{len(STATES)}")

    if failed_states:
        logger.warning(f"Failed/empty states: {', '.join(failed_states)}")
